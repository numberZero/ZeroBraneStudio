...
OPENSSL_BASENAME="openssl-1.1.1c"
...
if [ $BUILD_LUASEC ]; then
  ...
  gcc $BUILD_FLAGS -o "$INSTALL_DIR/lib/lua/$LUAV/ssl.dll" \
    ...
    src/luasocket/{timeout.c,buffer.c,io.c,wsocket.c} src/{config.c,context.c,ec.c,x509.c,ssl.c} -Isrc "$INSTALL_DIR/bin/libssl-1_1.dll" "$INSTALL_DIR/bin/libcrypto-1_1.dll" -lws2_32 -lgdi32 -llua$LUAV \
